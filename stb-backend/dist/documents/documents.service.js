...
function drawSTBHeader(doc, title, subtitle) {
...
function drawField(doc, label, value, x = 50, wide = false) {
...
function drawTwoFields(doc, label1, val1, label2, val2) {
    const col1X = 50;
...
let DocumentsService = class DocumentsService {
    ...
    async autoGenerateOnboardingDocuments(employeeId) {
        const types = [
    ...
    async findByEmployee(employeeId, year) {
        const filter = { employeeId: new mongoose_2.Types.ObjectId(employeeId), isActive: true };
        if (year)
            filter.year = year;
    ...
    async getStats(employeeId) {
        const docs = await this.documentModel.find({ employeeId: new mongoose_2.Types.ObjectId(employeeId), isActive: true }).exec();
...
], DocumentsService.prototype, "handleEmployeeCreatedEvent", null);
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_1ST_DAY_OF_MONTH_AT_MIDNIGHT),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], DocumentsService.prototype, "generateMonthlyPayslips", null);
