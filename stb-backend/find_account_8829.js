...
async function run() {
  ...
  const accounts = await db.collection('accounts').find({ accountNumber: /8829$/ }).toArray();
  for (const acc of accounts) {
    const emp = await db.collection('employees').findOne({ _id: acc.employeeId });
    console.log(`Account ${acc.accountNumber} belongs to ${emp?.nom} ${emp?.prenom} ID: ${emp?._id}`);
